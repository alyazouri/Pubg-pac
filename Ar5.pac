// ======================================================================
// PAC – PUBG Mobile الأردن: Smart Local Match + Auto Proxy Switch + HTTP/3
// ======================================================================
...
// ======================= PROXIES =======================
var PROXIES_CFG = [
  { ip: "91.106.109.12", socksPorts: [20000,20001,20003], httpPorts: [8080,8081,8085,8087,8088,8880], http3:true, activePlayers: 0 },
  { ip: "91.106.109.11", socksPorts: [20000,20001,20003], httpPorts: [8080,8081,8085,8087,8088,8880], http3:true, activePlayers: 0 }
];
...
function dedup(arr){ var seen={}, out=[]; for(var i=0;i<arr.length;i++){ var k=arr[i]; if(!seen[k]){ seen[k]=1; out.push(k); } } return out; }

// ======================= PROXY SELECTION =======================
function evaluateProxy(proxy){
  // يقلل score إذا البروكسي فيه لاعبين أردنيين فعليين
  return (proxy.activePlayers > 0) ? 0 : 5;
}
function sortProxies(){
  var arr = PROXIES_CFG.slice();
  arr.sort(function(a,b){
    var sa=evaluateProxy(a), sb=evaluateProxy(b);
...
function selectBestProxy(){
  ...
  return ordered[0]; // دائماً البروكسي الأقل score (نشاط أعلى)
}
...
function buildProxyChain(){ return buildTokens(selectBestProxy()).join("; "); }
