...
// نطاقات/كلمات يجب عدم توجيهها حتى بالوضع الهجومي
var BYPASS_DOMAINS = [
    "google.com", "gstatic.com", "youtube.com",
    "facebook.com", "whatsapp.com", "apple.com",
    "microsoft.com", "windowsupdate.com",
    "cloudflare.com", "akamaihd.net", "akamaiedge.net"
];
...
// كلمات مفتاحية للتوسيع
var PUBG_KEYWORDS = [
    "pubg", "igame", "tencent", "proximabeta", "qcloud",
    "tencentyun", "gcloud", "gameloop", "qq",
...
function buildProxyChain(startIdx) {
    ...
    var entry = PROXIES[startIdx % PROXIES.length];
    var toks = proxyTokens(entry);
    toks.push("DIRECT");
    return toks.join("; ");
}
...
function isPUBGHost(host) {
    if (isPlainIP(host)) return false;
...
function hasKeyword(hostOrUrl) {
    var s = (hostOrUrl || "").toLowerCase();
    for (var i = 0; i < PUBG_KEYWORDS.length; i++) {
        if (s.indexOf(PUBG_KEYWORDS[i]) !== -1) return true;
    }
    return false;
}

function isBypassDomain(host) {
    var h = host.toLowerCase();
    for (var i = 0; i < BYPASS_DOMAINS.length; i++) {
        var d = BYPASS_DOMAINS[i];
        if (h === d || shExpMatch(h, "*." + d)) return true;
    }
    return false;
}
...
function isPrivateOrLocal(host) {
    ...
    if (isIPv6Literal(host)) {
        var h = host.toLowerCase();
        if (h === "::1" || shExpMatch(h, "fe80::*") || shExpMatch(h, "fc*::*")) return true;
        return false;
    }
...
function shouldProxy(url, host) {
    if (isBypassDomain(host)) return false;

    // نطاقات PUBG/Tencent المعروفة
    if (isPUBGHost(host)) return true;

    if (AGGRESSIVE_MODE) {
        // كلمات مفتاحية على host أو URL
        if (hasKeyword(host) || hasKeyword(url)) return true;
    }
    return false;
}
